...
* Script to finetune: 
    0) low-resolution (LR) baseline on UCF-101-ELR,
    1) high-resolution (HR) branch on UCF-101, or
    2) cross-resolution knowledge distillation (KD) model on UCF-101-ELR
* Backbone model: I3D
* Dataset: UCF-101-ELR
* LR baseline example usage:
    python train_ucf.py --lr=1e-4 --bs=16 --stride=1 --clip_size=64 --model_num=0 --ckpt_hr='' --ckpt_lr=''
* HR (high-resolution) branch example usage:
    python train_ucf.py --lr=1e-4 --bs=16 --stride=1 --clip_size=64 --model_num=1 --ckpt_hr='' --ckpt_lr=''
* KD (knowledge distillation) model example usage:
    python train_ucf.py --lr=1e-4 --bs=16 --stride=1 --clip_size=64 --model_num=2 --ckpt_hr=models/hr-branch.pt --ckpt_lr=''
"""
...
def train(model_num, init_lr, batch_size, stride, clip_size, num_epochs, save_dir, root, train_split, test_split):
    writer = SummaryWriter()
    dataloaders = get_dataloaders(model_num, root, stride, clip_size, batch_size, train_split, test_split) 
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for phase in ['train', 'test']:
            ...
            if phase == 'train':
                ...
                print('test elapsed time: {}'.format(time.strftime("%H:%M:%S", time.gmtime(elapsed_time))))
            # --------------------------------------------------------------------
...
# ------------------------------------- HELPERS ------------------------------------------
def get_dataloaders(model_num, root, stride, clip_size, batch_size, train_split, test_split):
    print('Getting training dataset...')
    num_workers = 8

    def single(trans):
        train_dataset = UCF_Dataset(root, split_file=train_split,
                                    clip_size=clip_size, stride=stride,
                                    is_test=False, transform=trans)
        train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size,
                                                       shuffle=True, num_workers=num_workers,
                                                       pin_memory=True)
        print('Getting testing dataset...')
        
        test_dataset = UCF_Dataset(root, split_file=test_split,
                                   clip_size=clip_size, stride=stride,
                                   is_test=True, transform=trans)
    ...
    def pair(trans_hr, trans_lr):
        train_dataset = UCF_Dataset(root, split_file=train_split,
                                    clip_size=clip_size, stride=stride,
                                    is_test=False, transform=trans_lr,
                                    transform_hr=trans_hr, is_kd=True)
        train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size,
                                                       shuffle=True, num_workers=num_workers,
                                                       pin_memory=True)
        print('Getting testing dataset...')
        ...
        test_dataset = UCF_Dataset(root, split_file=test_split,
                                   clip_size=clip_size, stride=stride,
                                   is_test=True, transform=trans_lr,
                                   transform_hr=trans_hr, is_kd=True)
        test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size,
                                                      shuffle=False, num_workers=num_workers,
                                                      pin_memory=True)    
...
if __name__ == '__main__':
    if len(sys.argv) < len(vars(args))+1:
        ...
        train(model_num=MODEL_NUM, init_lr=LR, batch_size=BATCH_SIZE, stride=STRIDE,
              ...
              root='/vision/group/video/scratch/ucf101_old/two_stream_flow_frame',
              train_split='/vision/group/video/scratch/ucf101_old/trainlist01.txt',
              test_split='ucf_test_split.txt')
