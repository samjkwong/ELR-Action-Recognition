...
* Example usage: python eval_i3d.py --bs=16 --stride=2 --clip_size=64 --checkpoint_path=models/baseline-ucf.pt
"""
...
def test(root, batch_size, stride, clip_size, test_split):
    dataloader = get_dataloader(root, stride, clip_size, batch_size, test_split) 
...
# ------------------------------------- HELPERS ------------------------------------------
def get_dataloader(root, stride, clip_size, batch_size, test_split):
    ...
    test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=True, num_workers=8, pin_memory=True)
...
if __name__ == '__main__':
    if len(sys.argv) < len(vars(args))+1:
        ...
        test(root='/vision/group/video/scratch/ucf101_old/two_stream_flow_frame', batch_size=BATCH_SIZE,
             stride=STRIDE, clip_size=CLIP_SIZE, test_split='ucf_test_split.txt')
